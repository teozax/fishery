...
typedef struct {
    ...
    XNet_holes_detection_uio_map maps[ MAX_UIO_MAPS ];
} XNet_holes_detection_uio_info;

/***************** Variable Definitions **************************************/
static XNet_holes_detection_uio_info uio_info;
...
static int uio_info_read_name(XNet_holes_detection_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNet_holes_detection_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNet_holes_detection_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNet_holes_detection_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNet_holes_detection_Initialize(XNet_holes_detection *InstancePtr, const char* InstanceName) {
	XNet_holes_detection_uio_info *InfoPtr = &uio_info;
